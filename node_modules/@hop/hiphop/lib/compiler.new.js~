...
/*    Last change :  Fri Feb  7 07:57:58 2025 (serrano)                */
...
function makeCounterNet(env, ast_node, type,
   ...
   try {
      let init_val = init_counter_func();
      if (init_val > 0) {
...
ast.Fork.prototype.makeCircuit = function(env, sigtable) {
   ...
   function makeParallelCircuit(env, ast_node, childCircuits) {
	 ...
	 for (let k = 0; k < max; k++) {
	    ...
	    let kList = ccutils.getKListChild(ast_node, ast.Fork, iChild, k);
	    for (let i = 0; i <= ast_node.depth; i++) {
	       if (kList[i]) {
   ...
   let ccs = this.children.map((c, i, arr) => c.makeCircuit(env, sigtable));
...
ast.Trap.prototype.makeCircuit = function(env, sigtable) {
   ...
   function makeTrapCircuit(env, ast_node, child_interface) {
	 ...
	 return child_interface;
      ...
      let k0_list_child = ccutils.getKListChild(ast_node, ast.Trap, child_interface, 0);
      let trap_gate_list = ccutils.getKListChild(ast_node, ast.Trap,
					    child_interface, 2);
...
function makeSequenceCircuit(env, ast_node, childCircuits) {
   ...
   // get RES of children
   //
   let res = ccutils.getResChildren(ast_node, ast.Sequence, childCircuits);

   //
   // get SUSP of children
   //
   let susp = ccutils.getSuspChildren(ast_node, ast.Sequence, childCircuits);
...
function makePauseCircuit(env, ast_node) {
   ...
   let kill_list = [];
   let k_matrix = [[], []];
   let reg = new net.RegisterNet(ast_node, ast.Pause, "reg", 0);
   let and_to_or = net.makeAnd(ast_node, ast.Pause, "and_to_or");
...
ast.Atom.prototype.makeCircuit = function(env, sigtable) {
   ...
   for (let i = 0; i <= this.depth; i++) {
      ...
      let goNet = net.makeOr(this, ast.Atom, "go", i);
      let actionNet = new net.ActionNet(this, ast.Atom, "action", i,
...
function makeAbortCircuit(env, ast_node, childCircuit, force_non_immediate) {
   ...
   function makeAbortInner(env, ast_node, childCircuit) {
      let res = net.makeAnd(ast_node, ast.Abort, "abort_res");
      
      let and2 = net.makeAnd(ast_node, ast.Abort, "and2_negtest_res");
      let and3 = net.makeAnd(ast_node, ast.Abort, "and3_test_res");
...
function makeIfCircuit(env, ast_node, childCircuits) {
   ...
   // get RES of children
   //
   let res = ccutils.getResChildren(ast_node, ast.If, childCircuits);

   //
   // get SUSP of children
   //
   let susp = ccutils.getSuspChildren(ast_node, ast.If, childCircuits);
   ...
   for (let ki = 0; ki <= max_k; ki++) {
      let k_list_then = ccutils.getKListChild(ast_node, ast.If,
					 childCircuits[0], ki);
      let k_list_else = ccutils.getKListChild(ast_node, ast.If,
					 childCircuits[1], ki);
      k_matrix[ki] = [];
...
ast.Run.prototype.makeCircuit = function(env, sigtable) {
   ...
   let unbound = this.sigDeclList.filter(s => !s.signal);
...
ast.Local.prototype.makeCircuit = function(env, sigtable) {
   ...
   this.sigDeclList.forEach(sigprop => {
      if (!sigprop.alias) {
	 let s = new signal.Signal(this, sigprop, ast.Local, 
...
ast.Module.prototype.makeCircuit = function(env, sigtable) {
   let boot_reg = new net.RegisterNet(this, ast.Module, "global_boot_register", 0);
   ...
   for (let i in this.sigDeclList) {
	 ...
	 : null;
      let s = new signal.Signal(this, sigdecl, ast.Module, pre_reg, null, null, null);
   ...
   let global_sel = net.makeOr(this, ast.Module, "global_sel");
   let global_res = net.makeAnd(this, ast.Module, "global_res");
